...
) NESTED TABLE telephone STORE AS tTelephone;
/
...
/
CREATE OR REPLACE TYPE tJoueur AS OBJECT (
    refMembre REF tMembre
);
/
CREATE OR REPLACE TYPE tJoueursEquipe AS TABLE OF tJoueur;
/
CREATE OR REPLACE TYPE tEquipe AS OBJECT (
    nom varchar(50),
    joueurs tJoueursEquipe
);
/
